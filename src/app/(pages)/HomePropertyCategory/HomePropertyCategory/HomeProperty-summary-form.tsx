...
import { Check, ChevronLeft, Edit, Plus, Trash2 } from "lucide-react";
...
export default function PropertySummaryForm({
  ...
  const isPropertyDetailsComplete =
    formData.propertyDetails && formData.propertyDetails.name;
  ...
  const isFinalDetailsComplete =
    formData.finalDetails && formData.finalDetails.taxDetails;
    ...
    <div className="flex-1 max-w-3xl mx-auto w-full px-4 py-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-8">
        <div className="bg-white rounded-lg border border-gray-200 p-4 hover:shadow transition-shadow relative overflow-hidden">
          ...
          <div className="text-xs text-gray-500 space-y-1">
            ...
            <p>
              Name:{" "}
              <span className="font-medium">
                {formData.propertyDetails?.name || "Not set"}
              </span>
            </p>
            <p>
              Star rating:{" "}
              <span className="font-medium">
                {formData.propertyDetails?.starRating || "Not set"}
              </span>
            </p>
          </div>
          <button
        ...
        <div className="bg-white rounded-lg border border-gray-200 p-4 hover:shadow transition-shadow relative overflow-hidden">
          ...
          <h2 className="font-semibold text-lg mb-2">Rooms And Spaces</h2>
          <p className="text-sm text-gray-600 mb-3">
                ...
                {formData.rooms.map((room: any, index: number) => (
                  ...
                  >
                    <div className="flex items-center">
                      ...
                      {room.name} - {room.price?.currency} {room.price?.amount}
                      /night
                    </div>
